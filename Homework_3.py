...
hh = db.hh
id = 1
for vacancy in vacacies:
    try:
        hh.insert_one({'_id': id,
                        'name': vacancy['name'],
                        'min_salary': vacancy['min_salary'],
                        'max_salary': vacancy['max_salary'],
                        'currency': vacancy['currency'],
                        'link': vacancy['link'],
                        'site_sourсe': vacancy['site_sourсe']
                       })
        id += 1
    except dke:
        pass
...
def vacancy_find(salary, currency):
    for doc in hh.find({'$or': [{'min_salary': {'$gt': salary}},
